...
confirmBtn.addEventListener('click', event => {
    ...
    if (emailInput.value.length < 1 || !emailTrust) {
        event.preventDefault()
...
passwordDiv.addEventListener('click', event => {
    event.preventDefault()
...
usernameInput.addEventListener('keypress', event => {
    ...
    console.log(word)
    if ((word < 65 || word > 90) && (word < 97 || word > 122) && word != 32 && word != 45 && word != 95) {
        event.preventDefault()
    }
})
...
window.addEventListener('click', event => {
    if (event.target.id === 'eye-icon' || event.target.parentElement.id === 'eye-icon') {
        eyeIconContainer.classList.remove('show-icon')
        eyeSlashIconContainer.classList.add('show-icon')
        passwordInput.removeAttribute('type')
        passwordInput.setAttribute('type', 'password')
    }
    if (event.target.id === 'eye-slash-icon' || event.target.parentElement.id === 'eye-slash-icon') {
        eyeSlashIconContainer.classList.remove('show-icon')
